...
export function CylinderCarousel({
  ...
  // Prevent window scroll when wheeling over the carousel
  useEffect(() => {
    const el = stageRef.current;
    if (!el) return;
  ...
  const initItemWidth = itemWidth ?? itemSize;
  const initItemHeight = itemHeight ?? itemSize;

  const sizeX = itemWidth ?? Math.min(initItemWidth, (stageWidth * 0.65) / scaleSum);
  const sizeY = itemHeight ?? (initItemHeight * (sizeX / initItemWidth));

  const gap = stageWidth / (visibleItems + 1);
  
  // Math relating arc depth to radius: if R is large, arc is flatter.
  // Standard approximation: arc = sizeX^2 / (8 * R). Let's use a scale multiplier so a radius of 200-500 yields a pleasant curve.
  // Let's fallback to arcProp or sizeX * 0.35 if radius is not specified.
  const arc = radius 
    ? Math.max(5, (sizeX * sizeX) / (radius * 1.5)) 
    : (arcProp ?? sizeX * 0.35);
  ...
  const stageHeight = height ?? sizeY;
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          // clip-path, not overflow: it also clips the GPU-composited balls. Relaxed vertically for shadow.
          "relative w-full touch-none select-none outline-none [clip-path:inset(-100px_0)]",
