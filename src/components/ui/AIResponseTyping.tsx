...
export function AIResponseTyping({
  ...
  return (
    <div className="w-full max-w-2xl">
      <div className="relative rounded-[2rem] bg-white py-8 px-10 min-h-[100px] shadow-[0_8px_30px_rgb(139,69,19,0.06)]">
        ...
        {isThinking && (
          ...
          >
            <span className="text-stone-500 text-sm font-mono">Thinking</span>
            <div className="flex gap-1">
              {[0, 1, 2].map((i) => (
                <motion.div
                  key={i}
                  className="w-2 h-2 rounded-full bg-editorial-ink"
                  animate={{
                    scale: [1, 1.2, 1],
                    opacity: [0.5, 1, 0.5],
                  }}
        ...
        {displayedText && (
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="leading-relaxed whitespace-pre-wrap break-words font-serif italic text-base md:text-lg text-editorial-ink/80 tracking-wide"
          >
            {displayedText}
            {showCursor && isTyping && (
              <motion.span
                className="inline-block w-1.5 h-4 bg-editorial-ink ml-1 align-middle"
            ...
            {showCursor && !isTyping && !isThinking && displayedText && (
              <motion.span
                className="inline-block w-1.5 h-4 bg-editorial-ink ml-1 align-middle opacity-50"
            ...
            <motion.div
              ...
              animate={{
                background: [
                  ...
                  "linear-gradient(135deg, rgba(139, 69, 19, 0.05), transparent)",
                  "linear-gradient(135deg, transparent, transparent)",
                ],
              }}
              transition={{
      ...
      <div className="mt-3 flex items-center gap-2">
        <div className="flex items-center gap-2 text-xs text-stone-500 font-mono uppercase tracking-widest">
          <motion.div
            animate={{
            ...
            className="w-1.5 h-1.5 rounded-full bg-editorial-accent"
          />
          <span>
            {isThinking
              ? "AI is analyzing..."
